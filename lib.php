...
function block_ned_teacher_tools_assign_count_ungraded($assign, $graded, $students,
...
function block_ned_teacher_tools_quiz_count_ungraded($quizid, $graded, $students, $show='unmarked',
        ...
        if (function_exists($gradefunction) &&
            ...
            if ($modgrades) {
                ...
                if (is_array($modgrades->grades) && is_array($students)) {
                    ...
                    $ungradedfunction = 'block_ned_teacher_tools_' . $mod->modname . '_students_ungraded';
...
function block_ned_teacher_tools_count_unmarked_activities(&$course, $info='unmarked', $module='', $userid=0) {
    ...
    $groupstudents = false;
    if ($userid) {
        $groupstudents = block_ned_teacher_tools_mygroup_members($course->id, $userid);
    }
...
function block_ned_teacher_tools_count_notloggedin($course, $days) {
    $truants = block_ned_teacher_tools_get_notloggedin($course, $days);
    return count($truants);
}

function block_ned_teacher_tools_get_notloggedin($course, $days) {
    global $USER;
...
function block_ned_teacher_tools_get_failing($course, $percent) {
    global $DB;
...
function block_ned_teacher_tools_count_failing($course, $percent) {
    return count(block_ned_teacher_tools_get_failing($course, $percent));
}

function block_ned_teacher_tools_get_notsubmittedany($course, $since = 0, $count = false, $sections, $students) {
    global $DB;
...
function block_ned_teacher_tools_apply_grade_to_user($formdata, $userid, $attemptnumber, $assign) {
    global $USER, $CFG, $DB;

    $grade = $assign->get_user_grade($userid, true, $attemptnumber);
    $gradingdisabled = $assign->grading_disabled($userid);
    $gradinginstance = block_ned_teacher_tools_get_grading_instance($userid, $grade, $gradingdisabled, $assign);
    if (!$gradingdisabled) {
        if ($gradinginstance) {
    ...
    $feedbackplugins = block_ned_teacher_tools_load_plugins('assignfeedback', $assign);
...
function block_ned_teacher_tools_load_plugins($subtype, $assign) {
    global $CFG;
    $result = array();
...
function block_ned_teacher_tools_process_outcomes($userid, $formdata, $assign) {
    global $CFG, $USER;
...
function block_ned_teacher_tools_process_save_grade(&$mform, $assign, $context, $course, $pageparams, $gradingonly = true) {
    global $CFG;
    // Include grade form.
    ...
    if ($formdata = $mform->get_data()) {
        ...
        if ($instance->teamsubmission && $formdata->applytoall) {
            ...
            foreach ($members as $member) {
                ...
                block_ned_teacher_tools_apply_grade_to_user($formdata, $member->id, $attemptnumber. $assign);
                block_ned_teacher_tools_process_outcomes($member->id, $formdata, $assign);
            }
        } else {
            block_ned_teacher_tools_apply_grade_to_user($formdata, $userid, $attemptnumber, $assign);

            block_ned_teacher_tools_process_outcomes($userid, $formdata, $assign);
        }
        ...
        if ($shouldreopen && !$maxattemptsreached) {
            block_ned_teacher_tools_process_add_attempt($userid, $assign);
        }
    } else {
        return false;
    }
...
function block_ned_teacher_tools_view_single_grade_page($mform, $offset=0, $assign, $context,
    ...
    if ($participants) {
        ...
        if ($useridlist) {
            ...
            $useridlist = block_ned_teacher_tools_get_grading_userid_list($assign);
        }
        $useridlist = $arruser;
        $last = false;
    ...
    $allsubmissions = block_ned_teacher_tools_get_all_submissions($userid, $assign);
...
function block_ned_teacher_tools_view_submissions($mform, $offset=0, $showattemptnumber=null, $assign, $ctx, $cm, $course, $pageparams) {
    ...
    if (! has_capability('mod/assign:grade', $ctx)) {
        if (has_capability('block/ned_teacher_tools:viewreadonly', $ctx)) {
            $readonly = true;
        } else {
    ...
    $participants = optional_param('participants', '0', PARAM_INT);
    $arruser = block_ned_teacher_tools_count_unmarked_students($course, $cm, $pageparams['show'], $pageparams['sort']);
    ...
    if ($pageparams['show'] == 'unsubmitted') {
        ...
        foreach ($useridlist as $key => $userid) {
            ...
            list($allsubmissions, $allgrades, $attemptnumber, $maxattemptnumber)
                = block_ned_teacher_tools_get_submission_history_view($submission, $grade, $user, $showattemptnumber, $assign);
...
function block_ned_teacher_tools_render_assign_submission_history(assign_submission_history $history, $assignrenderer) {
    global $OUTPUT, $DB;
    $historyout = '';
...
function block_ned_teacher_tools_render_assign_submission_history_summary(assign_submission_history $history,
        ...
        if ($grade) {
            ...
            if ($i == $history->maxsubmissionnum) {
                $cell3->text = '<div style="float:left;">'.$cell3->text.'
                                ...
                                <a href="'.$CFG->wwwroot.'/blocks/ned_teacher_tools/fn_gradebook.php?courseid='.
...
function block_ned_teacher_tools_render_assign_submission_status(assign_submission_status $status, $assign, $user,
...
function block_ned_teacher_tools_get_all_submissions($userid, $assign) {
    global $DB, $USER;
...
function block_ned_teacher_tools_process_add_attempt($userid, $assign) {
    ...
    block_ned_teacher_tools_update_submission($newsubmission, $userid, false, $assign->get_instance()->teamsubmission, $assign);
    return true;
}

function block_ned_teacher_tools_update_submission(stdClass $submission, $userid, $updatetime, $teamsubmission, $assign) {
    global $DB;
    ...
    if ($result) {
        block_ned_teacher_tools_gradebook_item_update($submission, null, $assign);
    }
    return $result;
}

function block_ned_teacher_tools_gradebook_item_update($submission=null, $grade=null, $assign) {
    ...
    if ($submission != null) {
        ...
        $gradebookgrade = block_ned_teacher_tools_convert_submission_for_gradebook($submission);
...
function block_ned_teacher_tools_convert_submission_for_gradebook(stdClass $submission) {
...
function block_ned_teacher_tools_render_assign_attempt_history(assign_attempt_history $history) {
    $o = '';
...
function block_ned_teacher_tools_assignment_status($mod, $userid) {
...
function block_ned_teacher_tools_add_to_log_legacy($assign, $action = '', $info = '', $url='', $return = false) {
    ...
    call_user_func_array('block_ned_teacher_tools_add_to_log_legacy_', $args);
}

function block_ned_teacher_tools_get_block_config ($courseid, $blockname='ned_teacher_tools') {
    global $DB;
...
function block_ned_teacher_tools_build_ungraded_tree($courses, $supportedmodules, $classforhide='', $showzeroungraded=0, $maxcourse=10) {
    ...
    if (is_array($courses) && !empty($courses)) {
        ...
        foreach ($courses as $course) {
            ...
            $isingroup = block_ned_teacher_tools_isinagroup($course->id, $USER->id);
            if ($isingroup) {
                $userid = $USER->id;
            } else {
                $userid = 0;
            ...
            $courselink = $CFG->wwwroot . '/blocks/ned_teacher_tools/fn_gradebook.php?courseid=' .
            ...
            foreach ($supportedmodules as $supportedmodule) {
                if ($refreshmodefrontpage == 'pageload') {
                    $summary = block_ned_teacher_tools_count_unmarked_activities($course, 'unmarked', $supportedmodule, $USER->id);
                ...
                if ($numunmarked) {
                    ...
                    $moduletext .= '<div class="bullet" onclick="$(\'dd#cmid' . $supportedmodule .
                        ...
                        $supportedmodule . ' > ul\').toggleClass(\'block_ned_teacher_tools_hide\');"></div>';
            ...
            if (($totalungraded == 0) && !$showzeroungraded) {
                ...
                if ($totalungraded == 0) {
                    ...
                    $coursetext = '<dt id="courseid' . $course->id . '" class="cmod">
                        ...
                        $course->id . ' ~ dd\').toggleClass(\'block_ned_teacher_tools_hide\');"></div> 
                                 <a href="' . $courselink . '">' . $course->shortname . '</a> (' . $totalungraded . ')
                            </dt>';
                }
                $counter++;
    ...
    if (($counter >= $maxcourse) && ($refreshmodefrontpage != 'manual')) {
        $text .= "<div class='fn-admin-warning' >".get_string('morethan10', 'block_ned_teacher_tools')."</div>";
    }

    return $text;
}

function  block_ned_teacher_tools_get_course_category_tree($id = 0, $depth = 0) {
    global $DB, $CFG;
    require_once($CFG->libdir . '/coursecatlib.php');

    $viewhiddencats = has_capability('moodle/category:viewhiddencategories', context_system::instance());
    $categories = block_ned_teacher_tools_get_child_categories($id);
    $categoryids = array();
    foreach ($categories as $key => &$category) {
        ...
        if (empty($CFG->maxcategorydepth) || $depth <= $CFG->maxcategorydepth) {
            list($category->categories, $subcategories) = block_ned_teacher_tools_get_course_category_tree_($category->id, $depth + 1);
...
function  block_ned_teacher_tools_get_child_categories($parentid) {
    global $DB;
...
function  block_ned_teacher_tools_category_tree_form($structures, $categoryids='', $courseids='') {
    if ($categoryids == '0') {
    ...
    $content = '<ul id="course-category-tree" class="course-category-tree">
               ...
               <span class="ned-form-course-category">'.get_string('allcategories', 'block_ned_teacher_tools').'</span>';
    $content .= '<ul>';
    foreach ($structures as $structure) {
        $content .= '<li>';
        if (in_array($structure->id, $categoryids)) {
            $content .= block_ned_teacher_tools_checkbox_checked('category_'.$structure->id, 'category_'.$structure->id,
                    '_checkbox', $structure->id) . ' <span class="ned-form-course-category">'. $structure->name . '</span>';
        } else {
            $content .= block_ned_teacher_tools_checkbox('category_'.$structure->id, 'category_'.$structure->id,
                    '_checkbox', $structure->id) . ' <span class="ned-form-course-category">'. $structure->name . '</span>';
        }
        ...
        if ($structure->courses) {
            ...
            foreach ($structure->courses as $course) {
                if (in_array($course->id, $courseids)) {
                    $content .= html_writer::tag('li',  block_ned_teacher_tools_checkbox_checked('course_'.$course->id,
                            'course_'.$course->id, '_checkbox', $course->id) . ' <span class="ned-form-course">'.
                        $course->fullname.'</span>');
                } else {
                    $content .= html_writer::tag('li',  block_ned_teacher_tools_checkbox('course_'.$course->id,
                            'course_'.$course->id, '_checkbox', $course->id) . ' <span class="ned-form-course">'.
                        $course->fullname.'</span>');
                }
            }
        ...
        $content .= block_ned_teacher_tools_sub_category_tree_form($structure, $categoryids, $courseids);
        $content .= '</li>';
    }
    $content .= '</ul>';
    $content .= '</il>';
...
function  block_ned_teacher_tools_sub_category_tree_form($structure, $categoryids=null, $courseids=null) {
    $content = "<ul>";
    if ($structure->categories) {
        foreach ($structure->categories as $category) {
            $content .= '<li>';
            if (in_array($category->id, $categoryids)) {
                $content .= block_ned_teacher_tools_checkbox_checked('category_'.$category->id, 'category_'.$category->id,
                        '_checkbox', $category->id) . ' <span class="ned-form-course-category"">'. $category->name.'</span>';
            } else {
                $content .= block_ned_teacher_tools_checkbox('category_'.$category->id, 'category_'.$category->id,
                        '_checkbox', $category->id) . ' <span class="ned-form-course-category"">'. $category->name.'</span>';
            }
            if ($category->courses) {
                $content .= '<ul>';
            ...
            $content .= block_ned_teacher_tools_sub_category_tree_form($category, $categoryids, $courseids);
            $content .= '</li>';
        }
    }
    $content .= "</ul>";
...
function block_ned_teacher_tools_get_course_category_tree_($id = 0, $depth = 0) {
    global $DB, $CFG;
    $categories = array();
    $categoryids = array();
    $sql = context_helper::get_preload_record_columns_sql('ctx');
    ...
    foreach ($records as $category) {
        ...
        $categoryids[$category->id] = $category;
        if (empty($CFG->maxcategorydepth) || $depth <= $CFG->maxcategorydepth) {
            list($category->categories, $subcategories) = block_ned_teacher_tools_get_course_category_tree_(
                $category->id, $depth + 1);
            foreach ($subcategories as $subid => $subcat) {
                $categoryids[$subid] = $subcat;
            }
...
function block_ned_teacher_tools_checkbox($name, $id , $class, $value) {
...
function block_ned_teacher_tools_checkbox_checked($name, $id , $class, $value) {
    return html_writer::empty_tag('input', array(
            'value' => $value, 'type' => 'checkbox', 'id' => $id, 'name' => $name, 'class' => $class, 'checked' => 'checked'
        )
    );
}
function block_ned_teacher_tools_footer() {
    global $OUTPUT;
    ...
    $output = html_writer::div(
        ...
        html_writer::div(
            get_string('version', 'block_ned_teacher_tools') . ': ' .
...
function block_ned_teacher_tools_get_selected_courses($category, &$filtercourses) {
    if ($category->courses) {
        foreach ($category->courses as $course) {
    ...
    if ($category->categories) {
        foreach ($category->categories as $subcat) {
            block_ned_teacher_tools_get_selected_courses($subcat, $course);
        }
    }
};

function block_ned_teacher_tools_get_setting_courses() {
    global $DB;

    $filtercourses = array();
    $coursewithblockinstances = array();
    $allcourseswithblock = get_config('block_ned_teacher_tools', 'allcourseswithblock');
    $includehiddencourses = get_config('block_ned_teacher_tools', 'includehiddencourses');
    ...
    if ($allcourseswithblock) {
        $sql = "SELECT ctx.instanceid  
                 ...
                 WHERE ctx.id IN (SELECT bi.parentcontextid 
                                    FROM {block_instances} bi 
                                   WHERE bi.blockname = 'ned_teacher_tools') 
    ...
    if (empty($configcategory) && empty($configcourse)) {
        ...
        foreach ($selectedcategories as $categoryid) {
            ...
            if ($categorystructure = block_ned_teacher_tools_get_course_category_tree($categoryid)) {
                foreach ($categorystructure as $category) {
                    ...
                    if ($category->categories) {
                        foreach ($category->categories as $subcategory) {
                            block_ned_teacher_tools_get_selected_courses($subcategory, $filtercourses);
                        }
                    }
                }
            }
...
function block_ned_teacher_tools_cache_course_data ($courseid, progress_bar $progressbar = null) {
    global $DB, $USER, $CFG;
    ...
    foreach ($cachecourses as $filtercourse) {
        if ($course = $DB->get_record('course', array('id' => $filtercourse))) {
            if (get_config('block_ned_teacher_tools', 'cachedatalast_'.$course->id) === 0) {
                continue;
            }
            set_config('cachedatalast_'.$course->id, 0, 'block_ned_teacher_tools');
            $DB->execute("UPDATE {block_ned_teacher_tools_cach} SET expired = ? WHERE courseid = ?",
            ...
            set_config('cachedatalast_'.$course->id, time(), 'block_ned_teacher_tools');
        }
        $counter++;
        if (!is_null($progressbar)) {
            $donepercent = floor($counter / $numberofitems * 100);
...
function block_ned_teacher_tools_human_timing ($time) {
    $time = time() - $time;
    $time = ($time < 1) ? 1 : $time;
    $tokens = array (
        31536000 => 'year',
...
function block_ned_teacher_tools_gradebook_grade ($itemid, $userid) {
    global $DB;
...
function block_ned_teacher_tools_isinagroup($courseid, $userid) {
    global $DB;
...
function block_ned_teacher_tools_mygroup_members($courseid, $userid) {
    global $DB, $CFG;
    ...
    if ($groups = $DB->get_records_sql($sql, array($courseid, $userid))) {
        foreach ($groups as $group) {
            if ($groupmembers = block_ned_teacher_tools_groups_get_members_student($group->id, $courseid)) {
...
function block_ned_teacher_tools_groups_get_members_student($groupid, $courseid, $fields='u.*',
...
function block_ned_teacher_tools_groups_print_course_menu($course, $urlroot, $return=false, $activegroup=false) {
    global $USER, $OUTPUT;
...
function block_ned_teacher_tools_assignment_get_notsubmittedany($courseid, $id = "0", $users = null, $timestart) {
    global $DB;
...
function block_ned_teacher_tools_teacher_courses($userid) {
    global $DB;
...
function block_ned_teacher_tools_frontapage_cache_update_time($userid) {
    $time = false;

    $filtercourses = block_ned_teacher_tools_get_setting_courses();

    if ($teachercourses = block_ned_teacher_tools_teacher_courses($userid)) {
        $counter = 0;
        foreach ($teachercourses as $teachercourse) {
            if (in_array($teachercourse->courseid, $filtercourses)) {
                $counter++;
                $coursecache = get_config('block_ned_teacher_tools', 'cachedatalast_'.$teachercourse->courseid);
